...
#include <ntk/camera/openni_grabber_internals.hxx>

//QMutex ntk::ONIGrabber::m_ni_mutex;
...
{
    ntk::ONIGrabber* grabber = reinterpret_cast<ntk::ONIGrabber*>(pCookie);
...
ONIGrabber :: ONIGrabber(const std::string& file_path) :
    m_file_path(file_path),
	m_file_name(file_path.substr(file_path.find_last_of("\\")+1)),
...
void ONIGrabber :: setDefaultBayerMode()
{
	//MP
	//It is a ONI file recorded by kinect
    //if (m_driver.deviceInfo(m_camera_id).camera_type == "SensorKinect")
        setCustomBayerDecoding(true);
}

void ONIGrabber :: setIRMode(bool ir)
{
...
void ONIGrabber :: setSubsamplingFactor(int factor)
{
...
const std::string ONIGrabber :: DEFAULT_XML_CONFIG_FILE =  "config/NestkConfig.xml";

void ONIGrabber :: set_xml_config_file(const std::string & xml_filename)
{
    m_xml_config_file = xml_filename;
}

bool ONIGrabber :: connectToDevice()
{
	...
	//MP
	//xn::Context Initialization
	status = m_context.Init();
	CHECK_RC(status, "Init");

	status = m_context.OpenFileRecording(m_file_path.data(), m_player);
	CHECK_RC(status, "Open File");
    
	status = m_context.EnumerateProductionTrees(XN_NODE_TYPE_DEVICE, NULL, device_node_info_list);
    CHECK_RC(status, "Cannot enumerate devices.");
    ...
    ////CHECK_RC(status, "Create Device Node");
	...
	status = m_context.FindExistingNode(XN_NODE_TYPE_DEPTH, m_ni_depth_generator);
	if (status != XN_STATUS_OK){
		ntk_dbg(1) << "No depth generator found. Using a default one...";
		xn::MockDepthGenerator mockDepth;
		status = mockDepth.Create(m_context);
		CHECK_RC(status, "Create mock depth");

		// set some defaults
		XnMapOutputMode defaultMode;
		defaultMode.nXRes = 640;
		defaultMode.nYRes = 320;
		defaultMode.nFPS = 30;
		status = mockDepth.SetMapOutputMode(defaultMode);
		CHECK_RC(status, "set default mode");

		// set FOV
		XnFieldOfView fov;
		fov.fHFOV = 1.0225999419141749;
		fov.fVFOV = 0.79661567681716894;
		status = mockDepth.SetGeneralProperty(XN_PROP_FIELD_OF_VIEW, sizeof(fov), &fov);
		CHECK_RC(status, "set FOV");

		XnUInt32 nDataSize = defaultMode.nXRes * defaultMode.nYRes * sizeof(XnDepthPixel);
		XnDepthPixel* pData = (XnDepthPixel*)xnOSCallocAligned(nDataSize, 1, XN_DEFAULT_MEM_ALIGN);

		status = mockDepth.SetData(1, 0, nDataSize, pData);
		CHECK_RC(status, "set empty depth map");

		m_ni_depth_generator = mockDepth;
	}

	
	

    //status = m_ni_depth_generator.Create(m_context, &query);
	...
	//m_mirrored = xnIsMirrored(m_ni_depth_generator);
 //   status = m_context.SetGlobalMirror(m_mirrored);
 //   CHECK_RC(status, "Mirror");

	status = m_context.FindExistingNode(XN_NODE_TYPE_IMAGE, m_ni_rgb_generator);
    ...
    if (m_has_rgb)
    {
        ...
        //CHECK_RC(status, "Set RGB mode");

		//XnUInt64 high_res;
		//m_ni_rgb_generator.GetIntProperty("Resolution",high_res);
        ...
        //    CHECK_RC(status, "Set Frame Sync");
        //}

        //status = m_ni_ir_generator.Create(m_driver.niContext(), &query);
        //CHECK_RC(status, "Create infrared generator");
    ...
    if (m_track_users)
    {
		status = m_context.FindExistingNode(XN_NODE_TYPE_USER, m_ni_user_generator);
		//CHECK_RC(status, "USER GENERATOR");
        if(status){
			status = m_ni_user_generator.Create(m_context);
			CHECK_RC(status, "Create user generator");
		}

		status = m_context.FindExistingNode(XN_NODE_TYPE_HANDS, m_ni_hands_generator);
		//CHECK_RC(status, "HANDS GENERATOR");
        status = m_ni_hands_generator.Create(m_context);
        CHECK_RC(status, "Create hands generator");

		status = m_context.FindExistingNode(XN_NODE_TYPE_GESTURE, m_ni_gesture_generator);
		//CHECK_RC(status, "GESTURE GENERATOR2");//
        status = m_ni_gesture_generator.Create(m_context);
        CHECK_RC(status, "Create gestures generator");
    }
    ...
    if (m_track_users)
    {
        ...
        CHECK_RC(status, "User::StartGenerating");

        status = m_ni_hands_generator.StartGenerating();
        CHECK_RC(status, "Hands::StartGenerating");

        status = m_ni_gesture_generator.StartGenerating();
        CHECK_RC(status, "Gesture::StartGenerating");
    }

    waitAndUpdateActiveGenerators();

	//m_calib_data = new ntk::RGBDCalibration();
	//m_calib_data->loadFromFile("calibration-a00364a13380103a.yml");
	m_calib_data = NULL;
	//m_calib_data->loadFromFile("calibration-a00364a13380103a.yml");
...
bool ONIGrabber :: disconnectFromDevice()
{
    QMutexLocker ni_locker(&m_ni_mutex);
...
void ONIGrabber :: waitAndUpdateActiveGenerators()
{
...
void ONIGrabber :: estimateCalibration()
{
    XnPoint3D p;
    p.X = 0; p.Y = 0; p.Z = -1;
    ...
    while (!m_should_exit)
    {
        ...
        if (m_has_rgb)
        {
            if (m_get_infrared)
            {
                ...
                for (unsigned int i = 0; i < irMD.XRes()*irMD.YRes(); ++i)
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    uchar* raw_rgb_ptr = m_current_image.rawRgbRef().ptr<uchar>();
                    for (unsigned int i = 0; i < rgbMD.XRes()*rgbMD.YRes()*3; i += 3)
                        for (int k = 0; k < 3; ++k)
                        {
        ...
        if (m_track_users)
        {
            ...
            for (unsigned int i = 0; i < sceneMD.XRes()*sceneMD.YRes(); ++i)
            {
...
void ONIGrabber :: newUserCallback(XnUserID nId)
{
...
void ONIGrabber :: lostUserCallback(XnUserID nId)
{
    ntk_dbg(1) << cv::format("Lost User %d\n", nId);
}

// Callback: Detected a pose
void ONIGrabber :: userPoseDetectedCallback(XnUserID nId)
{
...
void ONIGrabber :: calibrationStartedCallback(XnUserID nId)
{
    ntk_dbg(1) << cv::format("Calibration started %d\n", nId);
}

// Callback: Finished calibration
void ONIGrabber :: calibrationFinishedCallback(XnUserID nId, bool success)
{
...
//struct ntk::ONIDriver::Config
//{
//    Config (ONIDriver* that)
//        : that(that)
//    {
...
//
//
//ntk::ONIDriver::ONIDriver() : m_config(new Config(this))
//{
...
//}
//
//void ntk::ONIDriver :: checkXnError(const XnStatus& status, const char* what) const
//{
//    if (status != XN_STATUS_OK)
//    {
...
//}
//#else
//void ntk::ONIDriver::findSerialNumbers()
//{
//    for (size_t i = 0; i < m_device_nodes.size(); ++i)
//    {
//        DeviceInfo& info = m_device_nodes[i];
...
//
//const char*
//ntk::ONIDriver::Config::configText = "\
//<OpenNI>\n\
//<Licenses>\n\
...
//#else
//const char*
//ntk::ONIDriver::Config::modulesText = "\
//<Modules>\n\
